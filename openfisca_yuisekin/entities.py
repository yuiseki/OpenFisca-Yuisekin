...
Household = build_entity(
    ...
    Calculate a variable applied to each individual of the group entity (e.g. calculate the '所得' of each member of the 'Household' with salaries = household.members("所得", period = MONTH); sum_salaries = household.sum(salaries)).
...
Person = build_entity(
    ...
    Variables like '所得' and '所得税' are usually defined for the entity 'Person'.

    Usage:
    Calculate a variable applied to a 'Person' (e.g. access the '所得' of a specific month with person("所得", "2017-05")).
